...
namespace Baitkm.BLL.Services.Announcements
{
    public class AnnouncementService : IAnnouncementService
    {
        ...
        public async Task<int> AddAsync(AddAnnouncementModel model, int userId, Language language)
        {
            ...
            if (model.AnnouncementResidentialType != AnnouncementResidentialType.Compound)
            {
            ...
            {
                if (model.CompoundItemIds.Any())
                {
                    foreach (var id in model.CompoundItemIds)
                    {
                        Announcement ann = _repository.GetById<Announcement>(id);
                        ann.AnnouncmentId = announcement.Id;
                        _repository.Update<Announcement>(ann);
                    }
                }
            }
        ...
        public async Task<int> AddCompoundItem(AddCompoundItemModel compoundItemModel, int userId)
        {
            var user = await _repository.FilterAsNoTracking<User>(u => u.Id == userId).FirstOrDefaultAsync();
            if (user == null)
                throw new Exception(_optionsBinder.Error().UserNotFound);
            if (string.IsNullOrEmpty(compoundItemModel.Price.ToString()))
                throw new Exception(_optionsBinder.Error().FillPrice);

            Announcement CompoundItem = new Announcement
            {
                Price = compoundItemModel.Price,
                Area = compoundItemModel.Area,
                BathroomCount = compoundItemModel.BathroomCount ?? 0,
                BedroomCount = compoundItemModel.BedroomCount ?? 0,
                Floor = compoundItemModel.Floor ?? default,
                NumberOfFloors = compoundItemModel.NumberOfFloors,
                SittingCount = compoundItemModel.SittingCount,
                UserId = user.Id,
                FurnishingStatus = compoundItemModel.FurnishingStatus,
                AnnouncementRentType = compoundItemModel.AnnouncementRentType,
                AnnouncmentId = compoundItemModel.AnnouncementId
            };
            _repository.Create(CompoundItem);
            if (compoundItemModel.Features.Any())
            {
                foreach (var it in compoundItemModel.Features)
                {
                    _repository.Create(new AnnouncementFeature
                    {
                        FeatureType = it,
                        AnnouncementId = CompoundItem.Id
                    });
                }
            }
            ...
            return CompoundItem.Id;
        }

        public async Task<bool> EditCompoundItem(AddCompoundItemModel compoundItemModel, int userId)
        {
            var user = await _repository.FilterAsNoTracking<User>(u => u.Id == userId).FirstOrDefaultAsync();
            if (user == null)
                throw new Exception(_optionsBinder.Error().UserNotFound);
            var announcement = await _repository.FilterAsNoTracking<Announcement>(a => a.Id == compoundItemModel.id && a.AnnouncmentId == compoundItemModel.AnnouncementId).FirstOrDefaultAsync();
            if (announcement == null)
                throw new Exception(_optionsBinder.Error().AnnouncementNotFound);
            if (announcement.UserId != userId && user.RoleEnum != Role.Admin)
            {
                throw new Exception(_optionsBinder.Error().NotHavePermission);
            }
            if (string.IsNullOrEmpty(compoundItemModel.Price.ToString()))
                throw new Exception(_optionsBinder.Error().FillPrice);
            var announcementFeatures = await _repository.Filter<AnnouncementFeature>(af =>
                af.AnnouncementId == compoundItemModel.id).ToListAsync();
            _repository.HardDeleteRange<AnnouncementFeature>(announcementFeatures);


            announcement.Price = compoundItemModel.Price;
            announcement.Area = compoundItemModel.Area;
            announcement.BathroomCount = compoundItemModel.BathroomCount ?? 0;
            announcement.BedroomCount = compoundItemModel.BedroomCount ?? 0;
            announcement.Floor = compoundItemModel.Floor ?? default;
            announcement.NumberOfFloors = compoundItemModel.NumberOfFloors;
            announcement.SittingCount = compoundItemModel.SittingCount;
            announcement.FurnishingStatus = compoundItemModel.FurnishingStatus;
            announcement.AnnouncementRentType = compoundItemModel.AnnouncementRentType;
        ...
        public async Task<bool> EditAsync(EditAnnouncementModel model, int userId, Language language, int announcementId)
        {
            ...
            if (user == null)
                throw new Exception(_optionsBinder.Error().UserNotFound);
            var announcement = await _repository.FilterAsNoTracking<Announcement>(a => a.Id == announcementId).FirstOrDefaultAsync();
            if (announcement == null)
                throw new Exception(_optionsBinder.Error().AnnouncementNotFound);
            if (announcement.UserId != userId && user.RoleEnum != Role.Admin)
            {
            ...
            if (model.Lat > 0 && model.Lng > 0)
            {
                ...
                var addressEn = model.AddressEn;
                var addressAr = model.AddressAr;
                ...
                if (country == null && result.Country != null)
                {
                    ...
                    _repository.SaveChanges();
                    announcement.CountryId = country.Id;
                }
            ...
            announcement.MeterPrice = model.MeterPrice;
            ...
            announcement.Floor = model.Floor;
            ...
            announcement.WhatsApp = model.WhatsApp;
            announcement.WhatsAppCountryCode = model.WhatsAppCountryCode;
            announcement.IsProfileNumber = model.IsProfileNumber;
            announcement.RealAgentCompanyName = model.RealAgentCompanyName;
        ...
        public async Task<bool> UploadCallback(string path, int announcementId, AttachmentType announcementPhotoType, string type)
        {
            ...
            Language language, string deviceId)
        {
            ...
            string UserWhatsAppNumberCountryCode = "";
            double UserRate = 0;
            Currency currency;
            Guest guest = null;
            var user = await _repository.FilterAsNoTracking<User>(u => u.Id == userId).FirstOrDefaultAsync();
            ...
            var announcementUser = await _repository.FilterAsNoTracking<User>(u => u.Id == announcement.UserId).FirstOrDefaultAsync();

            UserWhatsAppNumber = announcementUser.Phone;
            UserDescription = announcementUser.Description;
            UserWhatsAppNumberCountryCode = announcementUser.PhoneCode;
            UserRate = announcementUser.Rate;
            ...
            var result = await _repository.FilterAsNoTracking<Announcement>(x => x.Id == announcementId)
                ...
                {
                    Id = a.Id,
                    UpdateDate = a.LastUpdate ?? a.CreatedDt,
                    IsProfileNumber = a.IsProfileNumber,
                    WhatsApp = a.IsProfileNumber == false ? a.WhatsApp : UserWhatsAppNumber,
                    WhatsAppCountryCode = a.IsProfileNumber == false ? a.WhatsAppCountryCode : UserWhatsAppNumberCountryCode,
                    UserDescritpion = UserDescription,
                    UserRate = UserRate,
                    ...
                    {
                        Photo = Path.GetExtension(h.File).ToLower() == ".mp4" ? Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaDownload, UploadType.AnnouncementPhoto, h.File, false, announcementId)
                        : Path.GetExtension(h.File).ToLower() == ".mov" ? Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaDownload, UploadType.AnnouncementPhoto, h.File, false, announcementId)
                        : Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaResize, UploadType.AnnouncementPhoto, h.File, ConstValues.Width, ConstValues.Height, false, announcementId),
                    ...
                    CurrencyId = 12,
                    RealAgentCompanyName = a.RealAgentCompanyName,
                    ...
                    ShareUrl = $"https://baitkmdetails.page.link/details?id={a.Id}",
            ...
            var Rating = await _repository.FilterAsNoTracking<UserRating>(r => r.UserId == userId && r.UserRated == result.UserId).FirstOrDefaultAsync();
            var currentRating = Rating?.Rat;
            result.currentRating = currentRating;

            result.compoundItems = result.AnnouncementResidentialType == AnnouncementResidentialType.Compound ?
                _repository.FilterAsNoTracking<Announcement>(x => x.AnnouncmentId == result.Id && x.IsDeleted != true).Select(s => new CompoundItemListViewModel
                {
                    Area = s.Area,
                    BathroomCount = s.BathroomCount,
                    Features = s.Features.Select(x => x.FeatureType).ToList(),
                    id = s.Id,
                    ...
                    AnnouncementRentType = s.AnnouncementRentType,
                    SittingCount = s.SittingCount,
                    IsDeleted = s.IsDeleted,
                    AnnouncementStatus = s.AnnouncementStatus,
                    Photo = new ImageOptimizer
                    {
                        Photo = Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaResize,
                            UploadType.AnnouncementBasePhoto, s.BasePhoto, ConstValues.Width, ConstValues.Height, false, 0),
                        PhotoBlur = Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaResize,
                            UploadType.AnnouncementBasePhoto, s.BasePhoto, 100, 100, true, 0)
                    },
                    Photos = s.Attachments.Where(y => y.AttachmentType == AttachmentType.OtherImages).Select(h => new ImageAndVideoOptimizer
                    {
                        Photo = Path.GetExtension(h.File).ToLower() == ".mp4" ? Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaDownload, UploadType.AnnouncementPhoto, h.File, false, s.Id)
                        : Path.GetExtension(h.File).ToLower() == ".mov" ? Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaDownload, UploadType.AnnouncementPhoto, h.File, false, s.Id)
                        : Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaResize, UploadType.AnnouncementPhoto, h.File, ConstValues.Width, ConstValues.Height, false, s.Id),
                        ThumbNail = Utilities.ReturnFilePath(ConstValues.MediaBaseUrl, ConstValues.MediaResize,
                            UploadType.AnnouncementPhoto, Path.GetExtension(h.File).ToLower() == ".mp4" ? $"{Path.GetFileNameWithoutExtension(h.File)}_thumb.jpg"
                            : Path.GetExtension(h.File).ToLower() == ".mov" ? $"{Path.GetFileNameWithoutExtension(h.File)}_thumb.jpg" : null,
                            ConstValues.Width, ConstValues.Height, false, s.Id),
                        type = h.Type
        ...
        {
            var user = await _repository.FilterAsNoTracking<User>(u => u.Id == userId).FirstOrDefaultAsync();
            var myAnnouncement = _repository.Filter<Announcement>(x => !x.IsDraft && x.UserId == user.Id && x.AnnouncmentId == null);
        ...
        {
            var myAnnouncement = _repository.Filter<Announcement>(x => !x.IsDraft && x.UserId == userId && x.AnnouncmentId == null);
            ...
            SortingType sortingType)
        {
            ...
            result = query.OrderByDescending(f => f.CreatedDt)
                .Select(f => new AnnouncementViewModel
                {
                    ...
                    ShareUrl = $"https://baitkmdetails.page.link/details?id={f.AnnouncementId}",
            ...
            foreach (var r in result)
            {
                r.compoundItems = r.AnnouncementResidentialType ==
                   AnnouncementResidentialType.Compound ? _repository.FilterAsNoTracking<Announcement>(x => x.AnnouncmentId == r.Id).Select(s => new CompoundItemListViewModel
                   {
                       Area = s.Area,
                       BathroomCount = s.BathroomCount,
                       Features = s.Features.Select(x => x.FeatureType).ToList(),
        ...
        public async Task<bool> WatchedAsync(int announcementId, int userId, string deviceId, Language language)
        {
            bool isFavourite;
            var announcement = await _repository.FilterAsNoTracking<Announcement>(a => a.Id == announcementId
                && a.AnnouncementStatus != AnnouncementStatus.Rejected && !a.User.IsBlocked).FirstOrDefaultAsync();
            ...
            if (user != null)
            {
                isFavourite = await _repository.Filter<ViewedAnnouncement>(x => x.AnnouncementId == announcementId &&
                    x.UserId == user.Id).AnyAsync();
                if (!isFavourite)
                    _repository.Create(new ViewedAnnouncement
                    {
        ...
        public async Task<bool> UnWatchedAsync(int announcementId, int userId, string deviceId, Language language)
        {
            var announcement = await _repository.FilterAsNoTracking<Announcement>(x => !x.IsDraft && !x.User.IsBlocked
                && x.Id == announcementId).FirstOrDefaultAsync();
            if (announcement == null)
            ...
            if (user != null)
            {
                var Watched = await _repository.Filter<ViewedAnnouncement>(x => x.AnnouncementId == announcementId
                    && x.UserId == user.Id).FirstOrDefaultAsync();
                _repository.HardDelete(Watched);
                await _activityService.AddOrUpdate(announcement, user.Id, false, ActivityType.UnFavorite);
            }
            else
            {
        ...
        public async Task<IEnumerable<AnnouncementViewModel>> WatchedListAsync(int userId, string deviceId, Language language,
            SortingType sortingType)
        {
            IEnumerable<AnnouncementViewModel> result = null;
            IQueryable<ViewedAnnouncement> query = null;
            Currency currency;
            Guest guest = new Guest();
            var user = await _repository.FilterAsNoTracking<User>(u => u.Id == userId).FirstOrDefaultAsync();
            if (user != null)
            {
                currency = await _repository.Filter<Currency>(c => c.Id == user.CurrencyId).FirstOrDefaultAsync();
                query = _repository.FilterAsNoTracking<ViewedAnnouncement>(x => x.UserId == user.Id && !x.User.IsBlocked &&
                     x.Announcement.AnnouncementStatus == AnnouncementStatus.Accepted && !x.Announcement.IsDeleted);
            }
            else
            {
                ...
                if (guest == null)
                    throw new Exception(_optionsBinder.Error().UserNotFound);
                currency = await _repository.Filter<Currency>(c => c.Id == guest.CurrencyId).FirstOrDefaultAsync();
                query = _repository.FilterAsNoTracking<ViewedAnnouncement>(x => x.GuestId == guest.Id
                    && x.Announcement.AnnouncementStatus != AnnouncementStatus.Hidden
                    && !x.Announcement.User.IsBlocked && !x.Announcement.IsDeleted);
            }
            result = query.Distinct().OrderByDescending(f => f.CreatedDt)
                .Select(f => new AnnouncementViewModel
                {
                    Id = f.AnnouncementId,
                    Features = f.Announcement.Features.Where(a => !a.IsDeleted).Select(a => a.FeatureType).ToList(),
            ...
            int userId, string deviceId, SortingType sortingType, Language language)
        {
            Currency currency;
            Guest guest = null;
            IQueryable<Announcement> announcementFeatured = null;
            var user = await _repository.FilterAsNoTracking<User>(u => u.Id == userId).FirstOrDefaultAsync();
            if (user == null)
            {
                guest = await _repository.FilterAsNoTracking<Guest>(g => g.DeviceId == deviceId).FirstOrDefaultAsync();
                if (guest == null)
                    throw new Exception(_optionsBinder.Error().UserNotFound);
                currency = await _repository.Filter<Currency>(c => c.Id == guest.CurrencyId).FirstOrDefaultAsync();
                announcementFeatured = _repository.Filter<Announcement>(x => !x.IsDraft && !x.User.IsBlocked && x.TopAnnouncement
                && x.AnnouncementStatus == AnnouncementStatus.Accepted);
            }
            else
            {
                currency = await _repository.Filter<Currency>(c => c.Id == user.CurrencyId).FirstOrDefaultAsync();
                announcementFeatured = _repository.Filter<Announcement>(x => 
                    !x.IsDraft 
                    && !x.User.IsBlocked 
                    && x.TopAnnouncement
                    && x.AnnouncementStatus == AnnouncementStatus.Accepted
            ...
            int announcementId, int userId, string deviceId, Language language)
        {
            ...
            query = query.Where(x => x.CityId == announcement.CityId);
            ...
            var announcements = query.Skip((model.Page - 1) * model.Count).Take(model.Count)
                ...
                .Select(a => new AnnouncementViewModel
                {
                    ...
                    CurrencyCode = currency.Code,
                    DisctrictName = a.DisctrictName,
                    ShareUrl = $"https://baitkmdetails.page.link/details?={a.Id}",
                    Lat = a.Lat,
                    Lng = a.Lng,
                    CreateDate = a.CreatedDt,
                    BalconyArea = a.BalconyArea,
        ...
        public async Task<PagingResponseForSuggesting<AnnouncementViewModel>> NearbyMobileAsync(MobileNearByModel model,
            decimal lat, decimal lng, int userId, string deviceId, Language language)
        {
            ...
            IQueryable<Announcement> query = _repository.Filter<Announcement>(x => !x.IsDraft && !x.User.IsBlocked
            ...
            (PagingRequestAnnouncementFilterModel model, int userId, string deviceId, Language language)
        {
            ...
            {
                AnnouncementCategories announcementCategory = _repository.Filter<AnnouncementCategories>(x => x.CategoryNameEn.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart()) || x.CategoryNameAr.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())).FirstOrDefault();
                if (announcementCategory != null)
                {
                    switch (announcementCategory.MasterCategoryName)
                    {
                        case "AnnouncementType":
                            if (model.AnnouncementFilter.AnnouncementType == null)
                            {
                                query = query.Where(
                                a => a.Title.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                                || a.TitleArabian.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                                || a.Id.ToString().Contains(model.AnnouncementFilter.Search.Trim())
                                || a.AnnouncementType == (AnnouncementType)announcementCategory.CategoryId
                                );
                            }
                            else
                            {
                            ...
                            break;
                        case "LandType":
                            if (model.AnnouncementFilter.LandType == null)
                            {
                                query = query.Where(
                                a => a.Title.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                                || a.TitleArabian.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                                || a.Id.ToString().Contains(model.AnnouncementFilter.Search.Trim())
                                || a.LandType == (LandType)announcementCategory.CategoryId
                                );
                            }
                            else
                            {
                            ...
                            break;
                        case "CommercialType":
                            if (model.AnnouncementFilter.CommercialType == null)
                            {
                                query = query.Where(
                                a => a.Title.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                                || a.TitleArabian.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                                || a.Id.ToString().Contains(model.AnnouncementFilter.Search.Trim())
                                || a.CommercialType == (CommercialType)announcementCategory.CategoryId
                                );
                            }
                            else
                            {
                        ...
                        case "AnnouncementEstateType":
                            if (model.AnnouncementFilter.AnnouncementEstateType == null)
                            {
                               ...
                               || a.TitleArabian.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                               || a.Id.ToString().Contains(model.AnnouncementFilter.Search.Trim())
                               || a.AnnouncementEstateType == (AnnouncementEstateType)announcementCategory.CategoryId
                               );
                            }
                            else
                            {
                            ...
                            break;
                        case "AnnouncementResidentialType":
                            if (model.AnnouncementFilter.AnnouncementResidentialType == null)
                            {
                              ...
                              || a.TitleArabian.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().TrimStart())
                              || a.Id.ToString().Contains(model.AnnouncementFilter.Search.Trim())
                              || a.AnnouncementResidentialType == (AnnouncementResidentialType)announcementCategory.CategoryId
                              );
                            }
                            else
                            {
                ...
                {
                    query = query.Where(
                        a => a.Title.ToLower().Contains(model.AnnouncementFilter.Search.ToLower().Trim())
            ...
            if (model.AnnouncementFilter.CityName != null)
            {
                City city = _repository.Filter<City>(c => c.Name == model.AnnouncementFilter.CityName || c.NameAr == model.AnnouncementFilter.CityName).Include(c => c.Country).FirstOrDefault();
                if (city != null)
                {
                    query = query.Where(x => x.CityId == city.Id);
                }
                else
                {
                    return null;
            ...
            if (model.AnnouncementFilter.CommercialType.HasValue)
            {
                query = query.Where(x => x.CommercialType == model.AnnouncementFilter.CommercialType);
                modelFilterCount += 1;
            }
            ...
            if (model.AnnouncementFilter.Features != null && model.AnnouncementFilter.Features.Count != 0)
            {
            ...
            IQueryable<AnnouncementListViewModel> announcements = null;
            List<AnnouncementListViewModel> announcementQueryList = new List<AnnouncementListViewModel>();
            List<AnnouncementListViewModel> announcementList = null;
            try
            {
                announcements = query
                   .OrderByDescending(a => a.CreatedDt)
                ...
                if (model.AnnouncementFilter.Lat != null && model.AnnouncementFilter.Lng != null && model.AnnouncementFilter.Distance != null)
                {
                    foreach (var a in announcements)
                    {
                        if (DistanceTo((double)a.Lat, (double)a.Lng, (double)model.AnnouncementFilter.Lat, (double)model.AnnouncementFilter.Lng) <= model.AnnouncementFilter.Distance)
                        {
                            announcementQueryList.Add(a);
                        }
                    }
                    modelFilterCount += 1;
                }

                if (announcementQueryList != null && announcementQueryList.Count() == 0)
                {
                    announcementQueryList = announcements.ToList();
                }
                ...
                if (model.AnnouncementFilter.SortingType.HasValue)
                {
                    ...
                    {
                        case SortingType.Featured:
                            announcementList = announcementQueryList.Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                        case SortingType.Newest:
                            announcementList = announcementQueryList.OrderByDescending(a => a.CreatedDt).Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                        case SortingType.PriceLow:
                            announcementList = announcementQueryList.OrderBy(a => a.Price).Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                        case SortingType.PriceHigh:
                            announcementList = announcementQueryList.OrderByDescending(a => a.Price).Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                        case SortingType.BedsLeast:
                            announcementList = announcementQueryList.OrderByDescending(a => a.BedroomCount).Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                        case SortingType.BedsMost:
                            announcementList = announcementQueryList.OrderBy(a => a.BedroomCount).Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                        case SortingType.MostView:
                            announcementList = announcementQueryList.OrderBy(a => a.View).Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                        case SortingType.TopRated:
                            announcementList = announcementQueryList.OrderByDescending(a => a.Rating).Skip((model.Page - 1) * model.Count).Take(model.Count).ToList();
                            break;
                    }
                }
                else
            ...
            foreach (var r in announcementList)
            {
                r.compoundItems = r.AnnouncementResidentialType ==
                   AnnouncementResidentialType.Compound ? _repository.FilterAsNoTracking<Announcement>(x => x.AnnouncmentId == r.Id).Select(s => new CompoundItemListViewModel
                   {
            ...
            return new PagingResponseAnnouncementFilter
            {
                ...
                ItemCount = announcementList.Count(),
                MapAreaFilterCount = announcementQueryList.Count(),
                Data = announcementList.ToList(),
                PageCount = Convert.ToInt32(Math.Ceiling(decimal.Divide(query.Count(), model.Count)))
            };
        }
            ...
            int userId, Language language, string deviceId)
        {
            ...
            if (!string.IsNullOrEmpty(model.Search))
            {
                AnnouncementCategories announcementCategory = _repository.Filter<AnnouncementCategories>(x => x.CategoryNameEn.ToLower().Contains(model.Search.ToLower().Trim()) || x.CategoryNameAr.ToLower().Contains(model.Search.ToLower().Trim())).FirstOrDefault();
                if (announcementCategory != null)
                {
                    switch (announcementCategory.MasterCategoryName)
                    {
                        case "AnnouncementType":
                            ...
                            {
                                query = query.Where(
                                a => a.Title.ToLower().Contains(model.Search.ToLower().Trim())
            ...
            {
                City city = _repository.Filter<City>(c => c.Name == model.CityName || c.NameAr == model.CityName).Include(c => c.Country).FirstOrDefault();
                if (city != null)
                {
                    query = query.Where(x => x.CityId == city.Id);
                }
        ...
        public async Task<DashboardViewModel> DashboardPendingListAdmin(int userId)
        {
            ...
            var RejectedAnnouncements = await
                _repository.FilterAsNoTracking<Announcement>(x => !x.IsDraft && x.AnnouncementStatus == AnnouncementStatus.Rejected).CountAsync();
            var ExpiredAnnouncements = await
                     _repository.FilterAsNoTracking<Announcement>(x => !x.IsDraft && x.AnnouncementStatus == AnnouncementStatus.Expired).CountAsync();
            var IsDraftAnnouncements = await
                    _repository.FilterAsNoTracking<Announcement>(x => x.IsDraft).CountAsync();
            var UserCount = await _repository.GetAll<User>().CountAsync();
            var UserCreateAnnouncemtsCount = await _repository.GetAll<Announcement>().GroupBy(x => x.UserId).CountAsync();
            ...
            return new DashboardViewModel
            {
                ...
                IsDraftAnnouncements = IsDraftAnnouncements,
                UnreadSupportMessagesCount = unreadSupportMessagesCount,
                AndroidCount = androidCount,
                IosCount = iosCount,
                WebCount = webCount,
                UserCount = UserCount,
                UserCreateAnnouncemtsCount = UserCreateAnnouncemtsCount
            };
        }
        ...
        public async Task<bool> RemoveFromTopListAsync(Language language, int announcementId)
        {
            var announcement = await _repository.Filter<Announcement>(x => !x.IsDraft && !x.User.IsBlocked
                && x.Id == announcementId).FirstOrDefaultAsync();

            announcement.TopAnnouncement = false;
            _repository.Update(announcement);
            await _repository.SaveChangesAsync();
            return true;
        }
        ...
        public async Task<bool> Delete(int announcementId, string userName, VerifiedBy verifiedBy)
        {
            ...
            if (announcement != null)
            {
                var announcementPhotos = await _repository.Filter<Attachment>(x => x.AnnouncementId == announcement.Id)
                    .ToListAsync();
                ...
                var notifications = await _repository.Filter<PersonNotification>(x => x.AnnouncementId == announcement.Id)
                   .ToListAsync();
                _repository.RemoveRange<Attachment>(announcementPhotos);
                _repository.RemoveRange<Favourite>(favoruites);
                _repository.RemoveRange<AnnouncementFeature>(announcementFeatures);
                _repository.RemoveRange<AnnouncementReport>(announcementReport);
                _repository.RemoveRange<PersonNotification>(notifications);
                var supportMessage = await _repository.Filter<SupportMessage>(x => x.SupportMessageBodyType ==
                SupportMessageBodyType.Announcement && x.MessageText.Contains(announcement.Id.ToString())).ToListAsync();
                _repository.RemoveRange<SupportMessage>(supportMessage);
                List<int> message = new List<int>();
                _repository.Remove<Announcement>(announcement);
            ...
            {
                throw new Exception("This Announcement Doesn't Exists");
                return false;
            }
        }
        ...
        public async Task<string> Share(int announcementId)
        {
            ...
            var html = $"https://baitkmdetails.page.link/details??id={announcementId}";
            return html;
        }
        ...
        {
            var announcementRatings = await _repository.FilterAsNoTracking<Rating>(a => a.AnnouncementId == announcementId)
        ...
        private static double DistanceTo(double lat1, double lng1, double lat2, double lng2, char unit = 'K')
        {
