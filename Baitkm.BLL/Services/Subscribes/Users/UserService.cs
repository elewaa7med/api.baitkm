...
namespace Baitkm.BLL.Services.Users
{
    public class UserService : IUserService
    {
        ...
        public async Task<bool> EditAsync(UserEditModel model, int userId, Language language)
        {
            ...
            //name = name.Replace("'", "&#39;");
            //dynamic property = new ExpandoObject();CityName
            ...
            else if (!string.IsNullOrEmpty(model.CityName))
            {
                City city = _repository.Filter<City>(c => c.Name.ToLower() == model.CityName.ToLower() || c.NameAr.ToLower() == model.CityName.ToLower()).Include(c => c.Country).FirstOrDefault();
                if (city == null)
                {
        ...
        {
            var caller = await _repository.Filter<User>(x => x.Email == userName && (x.RoleEnum == Role.Admin || x.Id == userId)).FirstOrDefaultAsync();
            if (caller == null)
            ...
            if (user != null)
            {
                ...
                var photoIds = await _repository.Filter<Attachment>(x =>
                    announcements.Select(a => a.Id).Contains((int)x.AnnouncementId)).ToListAsync();
                var favoriteIds = await _repository.Filter<Favourite>(x =>
                    announcements.Select(a => a.Id).Contains(x.AnnouncementId)).ToListAsync();
                ...
                var announcementReportIds = await _repository.Filter<AnnouncementReport>(x =>
                    announcements.Select(a => a.Id).Contains(x.AnnouncementId)).ToListAsync();
                _repository.RemoveRange<Attachment>(photoIds);
                _repository.RemoveRange<Favourite>(favoriteIds);
                _repository.RemoveRange<AnnouncementFeature>(announcementFeatureIds);
                _repository.RemoveRange<Conversation>(conversationsAnnouncementIds);
                _repository.RemoveRange<AnnouncementReport>(announcementReportIds);
                var verified = await _repository.Filter<Verified>(x => x.Email == user.Email
                    ...
                    .FirstOrDefaultAsync();
                var deviceTokens = await _repository.Filter<DeviceToken>(x => x.UserId == user.Id).ToListAsync();
                //var favourites = await _repository.Filter<Favourite>(x => x.UserId == user.Id).ToListAsync();
                var conversations = await _repository.Filter<Conversation>(x => x.AnnouncementCreatorId == user.Id
                    || x.QuestionerId == user.Id).ToListAsync();
                var supportConversations = await _repository.Filter<SupportConversation>(x => x.UserId == user.Id).ToListAsync();
                var supportMessages = await _repository.Filter<SupportMessage>(x => x.UserSenderId == user.Id).ToListAsync();
                var messages = await _repository.Filter<Message>(x => x.SenderId == user.Id).ToListAsync();
                var password = await _repository.Filter<Password>(x => x.UserId == user.Id).FirstOrDefaultAsync();
                var subscriptions = await _repository.Filter<SubscribeAnnouncement>(x => x.UserId == user.Id).ToListAsync();
                ...
                var sav = await _repository.Filter<SaveFilterFeature>(x =>
                ...
                _repository.RemoveRange<Announcement>(announcements);
                _repository.RemoveRange<DeviceToken>(deviceTokens);
                _repository.RemoveRange<SubscribeAnnouncement>(subscriptions);
                ...
                _repository.RemoveRange<SaveFilter>(saveFilters);
                _repository.RemoveRange<Conversation>(conversations);
                _repository.RemoveRange<SupportConversation>(supportConversations);
                _repository.RemoveRange<SupportMessage>(supportMessages);
                _repository.RemoveRange<Message>(messages);
                _repository.Remove<Password>(password);
                if (verified != null)
                    _repository.Remove<Verified>(verified);
                _repository.Remove<User>(user);
                await _repository.SaveChangesAsync();
            ...
            {
                throw new Exception(_optionsBinder.Error().NoUser);
        ...
        public async Task<bool> AddRatingAsync(AddUserRatingModel model, int userId, string deviceId)
        {
            Guest guest = null;
            ...
            var userRate = await _repository.Filter<User>(a => a.Id == model.UserId).FirstOrDefaultAsync();
            if (userRate == null)
                throw new Exception(_optionsBinder.Error().UserNotFound);

            if (user != null)
            {
                rating = await _repository.FilterAsNoTracking<UserRating>(r => r.UserId == user.Id
                    && r.UserRated == model.UserId).FirstOrDefaultAsync();
                if (rating != null)
                    throw new Exception("You already rate for this User");
                _repository.Create(new UserRating
                {
                    UserId = user.Id,
                    UserRated = model.UserId,
                    Rat = model.Rating
                });
            }
            else
            {
                rating = await _repository.FilterAsNoTracking<UserRating>(r => r.GuestId == guest.Id
                    && r.UserRated == model.UserId).FirstOrDefaultAsync();
                if (rating != null)
                    throw new Exception("You already rate for this User");
                _repository.Create(new UserRating
            ...
            var totalRating = await CalculateCompanyRating(userRate.Id, model.Rating);
            userRate.Rate = totalRating;
            _repository.Update(userRate);

            await _repository.SaveChangesAsync();
            return true;
        }
        private async Task<double> CalculateCompanyRating(int userId, int rating)
        {
            var announcementRatings = await _repository.FilterAsNoTracking<UserRating>(a => a.UserRated == userId)
