...
#pragma warning disable CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed                             
                Task.Factory.StartNew(() => Blur(Path.GetFileName(newDest), type), TaskCreationOptions.LongRunning);
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
                RemoveFile(newFile, type, announcementId);
                return !isRelativeRequested ? Path.GetFileName(newDest) : newDest;
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
                ...
                try
                {
                    ...
                    if (announcementPhotoType == AttachmentType.OtherImages && announcementId != 0)
                    {
                        if (!contentType.Contains("video"))
                        {
                            ...
                            await Callback(Path.GetFileName(newFilePath), announcementId, announcementPhotoType,"image");
                        }
                        else
                        {

                            await Callback(Path.GetFileName(dest), announcementId, announcementPhotoType, "video");
                    ...
                    if (announcementPhotoType == AttachmentType.OtherDocumentations && announcementId != 0
                        && type != UploadType.MessageFiles && (contentType.Contains("application") || contentType.Contains("plain")))
                    {
                        ConvertPdf(Path.GetFileName(newFile), type, announcementId);
                        await Callback(Path.GetFileName(dest), announcementId, announcementPhotoType,"Documentation");
                    }
                    //if (announcementId != 0)
        ...
        private  void GetThumbnail(string fileName, UploadType uploadType, int announcementId = 0)
        {
            ...
            using (var process = argument.CreateProcess(_webRootPath))
            {
                process.Start();
            }
        }

        private async Task Callback(string path, int announcementId, AttachmentType announcementPhotoType,string announcementType)
        {
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.TryAddWithoutValidation("announcementType", announcementType.ToString());
